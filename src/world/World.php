...
class World implements ChunkManager{
	 ...
	 * @phpstan-var array<int, true>
	 */
	private array $randomTickBlocks = []; //size (in blocks) of padding after both boundaries of the Y axis
	...
	/**
	 * @return AxisAlignedBB[]
	 * @phpstan-return list<list<AxisAlignedBB>>
	 * @internal
	 */
	public function getBlockCollisionBoxesForMovement(AxisAlignedBB $bb, float $step) : array{
		$minX = (int) floor($bb->minX);
		$minY = (int) floor($bb->minY);
		$minZ = (int) floor($bb->minZ);
		$maxX = (int) floor($bb->maxX);
		...
		$stepMinY = (int) floor($bb->minY + min(0, $step));
		$stepMaxY = (int) floor($bb->maxY + max(0, $step));
		...
		return [$collides, $collidesForStep];
	}
